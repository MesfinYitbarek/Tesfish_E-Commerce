    ...
    if (activeView === 'separated') {
      return {
        ...
        datasets: [
          {
            label: 'Companies',
            data: data.map(item => item.companies || 0),
          ...
          {
            label: 'Individuals',
            data: data.map(item => item.individuals || 0),
          ...
          {
            label: 'Customers',
            data: data.map(item => item.customers || 0),
      ...
      <div className="mt-6 grid grid-cols-3 gap-4">
        <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <p className="text-2xl font-bold text-blue-700 dark:text-blue-300">
            {formatNumber(latestData.companies || 0)}
          </p>
          <p className="text-xs text-blue-600 dark:text-blue-400">
            {calculateGrowth(latestData.companies, previousData.companies) >= 0 ? '+' : ''}{calculateGrowth(latestData.companies, previousData.companies)}% growth
          </p>
        </div>
        <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
          <p className="text-sm text-green-600 dark:text-green-400">Individuals</p>
          <p className="text-2xl font-bold text-green-700 dark:text-green-300">
            {formatNumber(latestData.individuals || 0)}
          </p>
          <p className="text-xs text-green-600 dark:text-green-400">
            {calculateGrowth(latestData.individuals, previousData.individuals) >= 0 ? '+' : ''}{calculateGrowth(latestData.individuals, previousData.individuals)}% growth
          </p>
        </div>
        <div className="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
          <p className="text-sm text-purple-600 dark:text-purple-400">Customers</p>
          <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">
            {formatNumber(latestData.customers || 0)}
          </p>
          <p className="text-xs text-purple-600 dark:text-purple-400">
            {calculateGrowth(latestData.customers, previousData.customers) >= 0 ? '+' : ''}{calculateGrowth(latestData.customers, previousData.customers)}% growth
          </p>
        </div>
      </div>
    </div>
